...
impl Podcast {
    ...
    pub fn read_all_podcasts2(cat: Option<String>) ->Result<Vec<Podcast>, Error>  {
        ...
        match cat {
            Some(cat_ref) =>{
                // full_statement = format!("SELECT * FROM podcasts  where category_id={} ORDER By name ASC;", cat_ref);
                full_statement = format!("SELECT *, count(episodes.viewed) as notviewed FROM podcasts LEFT JOIN episodes ON podcasts.podcast_id=episodes.podcast_id WHERE podcasts.category_id = {} AND episodes.viewed = 0 GROUP BY episodes.podcast_id ORDER BY podcasts.name;", cat_ref)
            },
            None => {
                full_statement = format!("SELECT *, count(episodes.viewed) as notviewed FROM podcasts LEFT JOIN episodes ON podcasts.podcast_id=episodes.podcast_id WHERE episodes.viewed = 0 GROUP BY episodes.podcast_id ORDER BY podcasts.name;");
                // full_statement = format!("SELECT * FROM podcasts ORDER By name ASC;");
            }
        }
    ...
    pub fn read_all_podcasts3(cat: Option<Category>) ->Result<Vec<Podcast>, Error>  {
        let db: DB = DB::new(Config::new());
        let conn: Connection = db.connect_to_database();
        let mut results: Vec<Podcast> = Vec::new();
        let full_statement: String;
        match cat {
            Some(cat_ref) =>{
                // full_statement = format!("SELECT * FROM podcasts  where category_id={} ORDER By name ASC;", cat_ref);
                full_statement = format!("SELECT *, count(episodes.viewed) as notviewed FROM podcasts LEFT JOIN episodes ON podcasts.podcast_id=episodes.podcast_id WHERE podcasts.category_id = {} AND episodes.viewed = 0 GROUP BY episodes.podcast_id ORDER BY podcasts.name;", cat_ref.id)
            },
            None => {
                full_statement = format!("SELECT *, count(episodes.viewed) as notviewed FROM podcasts LEFT JOIN episodes ON podcasts.podcast_id=episodes.podcast_id WHERE episodes.viewed = 0 GROUP BY episodes.podcast_id ORDER BY podcasts.name;");
                // full_statement = format!("SELECT * FROM podcasts ORDER By name ASC;");
    ...
    pub fn display(cat: Option<Category>) -> Result<u16, Error> {
        match Podcast::read_all_podcasts3(cat) {
            Ok(pods) =>{
                ...
                loop {
                    ...
                    row_iter = start;
                    while row_iter <= end {
                        println!("number {}. {} - {}", (row_iter + 1), pods[row_iter as usize].name, pods[row_iter as usize].notviewed);
                        row_iter += 1;
                    }
                    let mut line = String::new();
                    print!("Choice: ");
...
impl Clone for Podcast {
    fn clone(&self) -> Podcast {
        Podcast {
            ...
            notviewed: self.notviewed.clone()
        }
    }
}
